...
class CTHController extends Controller
{
    ...
    public function getCTH($manganh, $course_code){
        $cth = DB::table('chuong_trinh_hocs')
                ->where('Nganh', '=', $manganh)
                ->where('khoas', $course_code)
                ->orderby('MaMon','asc')->get();
        return $cth;
    }
    ...
    {
        DB::table('chuong_trinh_hocs')->where('MaMon', $request->MaMon)->where('Term', $request->Term)->where('Nganh', $request->Nganh)->where('khoas', $request->khoas)->delete();
        return 'Môn học '.$request->MaMon.' đã xóa thành công';
    }
}
